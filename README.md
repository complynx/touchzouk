...
For production, enable refresh tokens on the ZITADEL application. Admin login
requests the `offline_access` scope; the encrypted session cookie then rotates
the refresh token before the short-lived identity expires.
